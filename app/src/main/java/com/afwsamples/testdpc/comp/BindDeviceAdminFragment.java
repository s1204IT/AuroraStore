...
public class BindDeviceAdminFragment extends BaseSearchablePolicyPreferenceFragment
  ...
  public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
    ...
    mHideLauncherIconPreference = (DpcSwitchPreference) findPreference(KEY_HIDE_PO_LAUNCHER_ICON);
    mHideLauncherIconPreference.setOnPreferenceChangeListener(this);

    mInstallCaCertificatePreference = (DpcPreference) findPreference(KEY_INSTALL_CA_CERTIFICATE);
    ...
    mInstallCaCertificatePreference.setCustomConstraint(getCustomConstraint(NO_CUSTOM_CONSTRAINT));
  }
  ...
  private void refresh() {
    ...
    if (targetUsers.size() == 1) {
      ...
      Log.w(
          TAG, "Expecting to be able to bind to exactly one profile owner, but got " + targetUsers);
    }
    mBindDeviceAdminServiceHelper =
        new BindDeviceAdminServiceHelper<>(
            getActivity(),
    ...
    if (mHideLauncherIconPreference.isEnabled()) {
      mBindDeviceAdminServiceHelper.crossUserCall(
          service -> mHideLauncherIconPreference.setChecked(service.isLauncherIconHidden()));
    }
    ...
    switch (preference.getKey()) {
      case KEY_HIDE_PO_LAUNCHER_ICON:
        if (!mBindDeviceAdminServiceHelper.crossUserCall(
          ...
          Toast.makeText(getActivity(), R.string.bind_to_profile_owner_failed, Toast.LENGTH_LONG)
              .show();
        }
        return true;
    }
  ...
  private boolean isManagedProfileOwner() {
    return mProfileOwnerUser != null && mUserManager.getUserProfiles().contains(mProfileOwnerUser);
  }
