...
/**
 * Displays a list of available components with a checkbox for enabling the component. All
 * components belonging to system packages are enabled by default and can't be disabled.
 */
public class AvailableComponentsInfoArrayAdapter extends ToggleComponentsArrayAdapter {
  ...
  public AvailableComponentsInfoArrayAdapter(
      Context context, List<ResolveInfo> resolveInfoList, List<String> permittedPackageNames) {
  ...
  /**
   * There are three cases where a given component in a profile is enabled. 1) There is no
   * restriction on the given component. 2) The given component's package name exist in the
   * permitted package list. 3) The given component belong to a system package.
   */
  @Override
  public boolean isComponentEnabled(ResolveInfo resolveInfo) {
    if (resolveInfo != null
        && resolveInfo.serviceInfo != null
        && !TextUtils.isEmpty(resolveInfo.serviceInfo.packageName)) {
      if (mPermittedPackageNames == null || isSystemApp(resolveInfo.serviceInfo.applicationInfo)) {
        return true;
      } else {
