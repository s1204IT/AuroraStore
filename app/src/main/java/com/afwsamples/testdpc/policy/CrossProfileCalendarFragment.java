...
public class CrossProfileCalendarFragment extends BaseSearchablePolicyPreferenceFragment
  ...
  public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
    ...
    mSetAllowedPackagesPreference =
        (DpcPreference) findPreference(CROSS_PROFILE_CALENDAR_SET_ALLOWED_PACKAGES_KEY);
    mSetAllowedPackagesPreference.setOnPreferenceClickListener(this);

    mAllowAllPackagesPreference =
        (DpcSwitchPreference) findPreference(CROSS_PROFILE_CALENDAR_ALLOW_ALL_PACKAGES_KEY);
  ...
  private void showSetPackagesDialog() {
    ...
    final EditText setPackagesEditText = (EditText) dialogView.findViewById(R.id.input);

    setPackagesEditText.setText(
        Joiner.on(",")
            .join(mDevicePolicyManager.getCrossProfileCalendarPackages(mAdminComponentName)));
        ...
        .setPositiveButton(
            ...
            (dialogInterface, i) -> {
              ...
              final Set<String> packageNames =
                  packageNamesString.isEmpty()
                      ? Collections.emptySet()
                      : new ArraySet<>(
                          Arrays.asList(packageNamesString.replace(" ", "").split(",")));
              mDevicePolicyManager.setCrossProfileCalendarPackages(
